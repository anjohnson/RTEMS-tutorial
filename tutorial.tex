...
I used \verb@/usr/local/rtems/rtems\rtemsToolVersion@ as the
installation target directory.
The location of the RTEMS source is not critical.  This document assumes
that the root of the RTEMs source tree is \verb@/usr/local/rtems/source@.
...
(\verb@/usr/local/rtems/rtems\rtemsToolVersion/bin@) is on your shell search path.
For shells
...
\begin{alltt}
../\BINUTILS/configure --target=powerpc-rtems\rtemsToolVersion --prefix=/usr/local/rtems/rtems\rtemsToolVersion
\end{alltt}
...
\begin{alltt}
../\GCC/configure --target=powerpc-rtems\rtemsToolVersion --prefix=/usr/local/rtems/rtems\rtemsToolVersion \verb@\@
...
../rtems-\rtemsVersion/configure \verb@--target=powerpc-rtems\rtemsToolVersion --prefix=/usr/local/rtems/rtems\rtemsToolVersion \@
...
\begin{alltt}
cd /usr/local/rtems/source/build
../rtems-\rtemsVersion/configure \verb@--target=m68k-rtems\rtemsToolVersion --prefix=/usr/local/rtems/rtems\rtemsToolVersion \@
  \verb@  --enable-cxx --enable-rdbg --disable-tests --enable-networking \@
