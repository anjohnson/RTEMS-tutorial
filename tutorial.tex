...
The suggested location for the installation directory is \verb@/opt/rtems@.
If you must place the root of the RTEMS installation in some other
location, it is still a good idea to make a symbolic link from \verb@/opt/rtems@
...
(\verb@/opt/rtems/bin@) is on your shell search path.
For shells
...
For example, here is how the patches can be applied to the gcc sources:
\begin{alltt}
cd \GCC
...
\begin{alltt}
../\BINUTILS/configure --target=powerpc-rtems --prefix=/opt/rtems
\end{alltt}
...
\begin{alltt}
../\GCC/configure --target=powerpc-rtems --prefix=/opt/rtems \verb@\@
...
../rtems-\rtemsVersion/configure \verb@--target=powerpc-rtems --prefix=/opt/rtems \@
...
more names on the command line.  For example, for the \verb@m68k-rtems@
architecture you could for a generic MC68360 system and an MVME-167 system
by:
\begin{alltt}
cd /usr/local/source/RTEMS/build
../rtems-\rtemsVersion/configure \verb@--target=m68k-rtems --prefix=/opt/rtems \@
  \verb@  --enable-cxx --enable-rdbg --disable-tests --enable-networking \@
...
\begin{verbatim}
export RTEMS_MAKEFILE_PATH=/opt/rtems/powerpc-rtems/mvme2100
\end{verbatim}
...
Some extensions still use the old configuration procedure so it's a good idea
...
the remote file access technique being used s described in the following section.
